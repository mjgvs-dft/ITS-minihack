...
def load_and_process_data(viajes_path='../data/input/encuesta-19/ViajesEODH2019.csv',
                         zat_path='../data/input/zat/ZAT.shp'):
    
    """Load and process transportation data and ZAT shapefile"""
    logger.info(f"Current Working Directory: {os.getcwd()}")
    # Read Viajesmex data
    viajes = pd.read_csv(viajes_path, sep=';', encoding='latin1')
    ...
    # Read the shapefile
    zat_data = gpd.read_file(zat_path)
    
    # Merge the origin journeys data with the shapefile data
    zat_data_merged = zat_data.merge(origin_journeys[origin_journeys['zat_origen'] != 0], 
                                     left_on='ZAT', 
                                     right_on='zat_origen', 
                                     how='left')
    ...
    return zat_data_merged

def create_folium_map(zat_data_merged):
    """Create a Folium map from the processed data"""
